...
MMA16816SmemLoader::loadX4(int mat0, int mat1, ArrayRef<Value> ptrs, Type matTy,
  ...
  if (canUseLdmatrix) {
    ...
    auto f161_1 = extract_element(bitcast(res1, vec_ty(f16_ty, 2)), i32_val(0));
    auto f161_2 = extract_element(bitcast(res1, vec_ty(f16_ty, 2)), i32_val(1));
    auto f162_1 = extract_element(bitcast(res2, vec_ty(f16_ty, 2)), i32_val(0));
    auto f162_2 = extract_element(bitcast(res2, vec_ty(f16_ty, 2)), i32_val(1));
    auto f163_1 = extract_element(bitcast(res3, vec_ty(f16_ty, 2)), i32_val(0));
...
MMA16816SmemLoader::MMA16816SmemLoader(
  ...
  std::cout << instrShape[order[1]] << std::endl;
  std::cout << matShape[order[1]] << std::endl;
  std::cout << loadOffsetInMat[order[1] - 1] << std::endl;
...
getLoadMatrixFn(Value tensor, const SharedMemoryObject &smemObj,
  ...
  auto load = [=, &rewriter, &vals](int a, int b) {
    ...
    vals[{a + 1, b + 1}] = ha3;
    // if (counter == 0)
    //   mlir::LLVM::vprintf("tid: %d, ha0: %f, ha1: %f, ha2: %f, ha3: "
    //                       "%f\n",
    //                       {thread, ha0, ha1, ha2, ha3}, rewriter);
  };
